...
//   if(edep==0.) return true;
  ...
  //Aiwu add: print the steps in plastic scintillator
  if(edep>0){SetHitEdep(number, edep); }
  if(GetHodoHitTime()[number]==0.0 && edep>0/* && (pdg==13 || pdg==-13)*/ ){ // make sure hit time is set only once
      SetHodoHitTime(number,hitTime);
      //SetHitEdep(number,edep);
      SetHitFlag(number,1);
      G4cout<<"In Plastic scintillator "<<number<<G4endl;
      G4cout<<"EvtID "<<eventID<<"\tTrackID "<<trackID<<"\tPdgCode "<<pdg
            <<"\tPos "<<preStepPoint->GetPosition().x()<<","<<preStepPoint->GetPosition().y()<<","<<preStepPoint->GetPosition().z()
            <<"\tedep "<<edep<<"\tTime "<<hitTime;
      if(aTrack->GetParentID()!=0){
         G4cout <<"\tProcess "<<aStep->GetPostStepPoint()->GetProcessDefinedStep()->GetProcessName()<<G4endl;	  
      }
      else{
          G4cout<<"\tparentID=0"<<G4endl;
      }
  }
